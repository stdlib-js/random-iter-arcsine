...
function iterator( a, b, options ) {
	...
	if ( !isNumber( a ) || isnan( a ) ) {
		throw new TypeError( format( '0pF6v', a ) );
	}
	if ( !isNumber( b ) || isnan( b ) ) {
		throw new TypeError( format( '0pF6w', b ) );
	}
	if ( a >= b ) {
		throw new RangeError( format( '0pFEJ', a, b ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pF2V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pF2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
